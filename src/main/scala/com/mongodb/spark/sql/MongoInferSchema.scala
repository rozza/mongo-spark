...
object MongoInferSchema extends Logging {
  ...
  def apply(mongoRDD: MongoRDD[BsonDocument]): StructType = {
    ...
    val sampleData: MongoRDD[BsonDocument] = if (sampleSize >= samplePoolSize) {
      singlePartitionRDD.appendPipeline(Seq(Aggregates.limit(samplePoolSize)))
    } else if (singlePartitionRDD.hasSampleAggregateOperator) {
      val appendedPipeline = if (singlePartitionRDD.readConfig.pipeline.isEmpty) {
