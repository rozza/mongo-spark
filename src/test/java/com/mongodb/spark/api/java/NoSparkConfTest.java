...
import com.mongodb.spark.api.java.sql.Character;
...
public final class NoSparkConfTest extends RequiresMongoDB {
    ...
    public void shouldBeAbleToUseConfigsWithDataFrames() {
        ...
        List<Character> characters = asList(new Character("Gandalf", 1000), new Character("Bilbo Baggins", 50));

        sqlContext.createDataFrame(jsc.parallelize(characters), Character.class).write()
                .options(getOptions())
                .format("com.mongodb.spark.sql")
                .save();

        List<Character> ds = sqlContext.read()
                .format("com.mongodb.spark.sql")
                .options(getOptions())
                .load()
                .as(Encoders.bean(Character.class))
                .collectAsList();
