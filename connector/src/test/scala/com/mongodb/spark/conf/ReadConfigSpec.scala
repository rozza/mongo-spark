...
class ReadConfigSpec extends FlatSpec with Matchers {
  ...
  "ReadConfig" should "have the expected defaults" in {
    ...
    val expectedReadConfig = ReadConfig("db", "collection", 64, "_id", "primary", None, 1000, 1) // scalastyle:ignore

    readConfig should equal(expectedReadConfig)
  }

  it should "be creatable from SparkConfig" in {
    val expectedReadConfig = ReadConfig("db", "collection", 1, "_id", "secondary", Some("local"), 150, 0.75) // scalastyle:ignore

    ReadConfig(sparkConf) should equal(expectedReadConfig)
  }

  it should "be able mixin user parameters" in {
    val expectedReadConfig = ReadConfig("db", "collection", 10, "ID", "secondaryPreferred", Some("majority"), 200, 0.9) // scalastyle:ignore
  ...
  it should "validate the values" in {
    ...
    an[IllegalArgumentException] should be thrownBy ReadConfig("db", "collection", samplingRatio = -1.0)
  ...
  val sparkConf = new SparkConf()
    ...
    .set("mongo.input.samplingRatio", "0.75")
