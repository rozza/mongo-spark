...
object MongoInferSchema {
  ...
  def apply(mongoRDD: MongoRDD[BsonDocument]): StructType = {
    ...
    // TODO handle pre 3.2
    val sampleRDD = mongoRDD.withPipeline(List(new Document("$sample", new Document("size", mongoRDD.readConfig.sampleSize))))
    val schemaData = if (mongoRDD.readConfig.samplingRatio > 0.99) {
      sampleRDD
    } else {
      sampleRDD.sample(withReplacement = false, mongoRDD.readConfig.samplingRatio, 1)
    }
