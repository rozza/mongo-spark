...
object ReadConfig {
  ...
  private val DefaultSamplingRatio: Double = 1.0
  ...
  def apply(sparkConf: SparkConf): ReadConfig = {
    ...
    ReadConfig(
      ...
      samplingRatio = sparkConf.getDouble(samplingRatioProperty, DefaultSamplingRatio)
    )
  }
...
case class ReadConfig(
    ...
    samplingRatio:      Double         = ReadConfig.DefaultSamplingRatio
) {
  ...
  require(samplingRatio > 0, s"samplingRatio ($samplingRatio) must be greater than 0")
  ...
  def withParameters(parameters: Map[String, String]): ReadConfig = {
    ReadConfig(
      ...
      samplingRatio = parameters.get("samplingRatio") match {
        case Some(ratio) => ratio.toDouble
        case None        => samplingRatio
      }
    )
  }
